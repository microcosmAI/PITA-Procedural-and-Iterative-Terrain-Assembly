...
class MinDistanceMujocoPhysicsRule(Rule):
    ...
    def __call__(
        ...
        # If there are collisions, we need to check if the object is colliding only with itself or with other objects
            ...
            for geom in attachement_frame.all_children()[0].find_all("geom"):
                all_ids.append(physics.data.geom(attachement_frame.full_identifier+geom.name).id)
            ...
            mask = (np.isin(physics.data.contact.geom1, all_ids) & ~np.isin(physics.data.contact.geom2, all_ids)) | (~np.isin(physics.data.contact.geom1, all_ids) & np.isin(physics.data.contact.geom2, all_ids))
